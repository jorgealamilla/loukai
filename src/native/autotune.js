...
class PitchDetector {
    ...
    detectPitch(buffer) {
        ...
        const startIndex = (this.historyIndex - this.bufferSize + this.inputHistory.length) % this.inputHistory.length;
...
class PitchShifter {
        ...
        for (let i = 0; i < length; i++) {
            ...
            for (const grain of this.grains) {
                if (grain.active && grain.position < this.frameSize) {
                    outputSample += grain.buffer[Math.floor(grain.position)] * this.window[Math.floor(grain.position)];
    ...
    processGrain(timeStretchRatio) {
        ...
        const startIndex = (this.inputIndex - this.frameSize + this.inputBuffer.length) % this.inputBuffer.length;
